...
def _tum_anahtarlar() -> list[str]:
    """GEMINI_API_KEY (+ _2, _3) — rotasyon için tüm anahtarlar. Free-tier 20/gün/proje
    limitini aşmak için: biri dolunca diğerine geçilir."""
...
def _api_anahtarini_oku() -> str:
    """Şu an aktif anahtar (rotasyonlu)."""
    global _ANAHTARLAR
    if not _ANAHTARLAR:
...
def _sonraki_anahtara_gec() -> bool:
    """Günlük kota (PerDay) dolunca sıradaki anahtara geç. Başka anahtar varsa True."""
    global _AKTIF_IDX, _ANAHTARLAR
    if not _ANAHTARLAR:
        _ANAHTARLAR = _tum_anahtarlar()
    if _AKTIF_IDX + 1 < len(_ANAHTARLAR):
        _AKTIF_IDX += 1
        print(f"[bridge] Günlük kota doldu → {_AKTIF_IDX + 1}. anahtara geçildi "
              f"({len(_ANAHTARLAR)} anahtar var)", flush=True)
        return True
    return False
